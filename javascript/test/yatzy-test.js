...
describe("One pair", function() {
    it("scores the sum of the highest pair", function() {
        assert.equal(6, new Yatzy(3,4,3,5,6).scorePairs(1));
        assert.equal(10, new Yatzy(5,3,3,3,5).scorePairs(1));
        assert.equal(12, new Yatzy(5,3,6,6,5).scorePairs(1));
    });
});

describe("Two pair", function() {
    it("scores the sum of the two pairs", function() {
        assert.equal(16, new Yatzy(3,3,5,4,5).scorePairs(2));
        assert.equal(16, new Yatzy(3,3,5,5,5).scorePairs(2));
    });
});

describe("Three of a kind", function() {
    it("scores the sum of the three of the kind", function() {
        assert.equal(9, new Yatzy(3,3,3,4,5).someOfaKind(3));
        assert.equal(15, new Yatzy(5,3,5,4,5).someOfaKind(3));
        assert.equal(9, new Yatzy(3,3,3,3,5).someOfaKind(3));
    });
});

describe("Four of a kind", function() {
    it("scores the sum of the four of the kind", function() {
        assert.equal(12, new Yatzy(3,3,3,3,5).someOfaKind(4));
        assert.equal(20, new Yatzy(5,5,5,4,5).someOfaKind(4));
        assert.equal(12, new Yatzy(3,3,3,3,3).someOfaKind(4));
    });
});
